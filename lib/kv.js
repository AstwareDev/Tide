...
function getPool() {
  if (!pool) {
    pool = new Pool({
      // POSTGRES_URL is auto-injected by the Vercel Supabase integration;
      // DATABASE_URL lets local dev / other providers override it.
      connectionString: process.env.DATABASE_URL || process.env.POSTGRES_URL,
      max: 1,
      // Supabase's pooler cert chain isn't in Node's default trust store.
