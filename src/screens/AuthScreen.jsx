...
export default function AuthScreen() {
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      setError(err.message || "Failed to connect. Make sure your Google credentials are configured.");
    } finally {
      setLoading(false);
    }
  };

  // Dev shortcut: bypass auth to preview UI
  const handleDevPreview = () => navigate("/inbox");
      ...
      <div className="w-full max-w-md px-8">
        ...
        <div className="space-y-3 mb-10">
          {[
            ...
            { icon: ShieldCheck, text: "Your email never leaves your device except to the AI" },
          ].map(({ icon: Icon, text }) => (
        ...
        <button
          ...
          disabled={loading}
          className="w-full py-3 px-4 bg-white text-gray-900 font-semibold rounded-xl hover:bg-gray-100 disabled:opacity-60 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2 text-sm"
        >
          {loading ? (
            <>
        ...
        <button
          ...
          className="w-full mt-3 py-2 text-gray-600 hover:text-gray-400 text-xs transition-colors"
        >
          Preview UI (dev only)
        </button>
      </div>
