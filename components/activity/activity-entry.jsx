...
export function ActivityEntry({ entry, onUndo }) {
  ...
  return (
    <div className="flex items-start gap-3 py-2.5 px-3 rounded-xl hover:bg-accent transition-colors group">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-baseline gap-1.5 mb-0.5">
          ...
          {entry.labelApplied && <span className="text-xs text-muted-foreground">→ {entry.labelApplied}</span>}
          <span className="text-xs text-muted-foreground">·</span>
          <span className="text-xs text-muted-foreground truncate">{entry.agentName}</span>
          {entry.undone && <span className="text-xs text-muted-foreground">(undone)</span>}
        </div>
        <p className="text-sm text-muted-foreground truncate">{entry.emailSubject}</p>
        <p className="text-xs text-muted-foreground truncate">{entry.emailFrom}</p>
        {entry.reason && <p className="text-xs text-muted-foreground mt-1 italic truncate">{entry.reason}</p>}
      </div>
      ...
      <span className="text-[11px] text-muted-foreground shrink-0 mt-0.5">{formatTime(entry.timestamp)}</span>
    </div>
  );
}
